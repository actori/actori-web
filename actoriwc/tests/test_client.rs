...
use actori_service::{map_config, pipeline_factory};
use actori_web::dev::{AppConfig, BodyEncoding};
use actori_web::http::Cookie;
use actori_web::middleware::Compress;
use actori_web::{
    http::header, test, web, App, Error, HttpMessage, HttpRequest, HttpResponse,
};
use actoriwc::error::SendRequestError;
...
async fn test_timeout() {
    let srv = test::start(|| {
        App::new().service(web::resource("/").route(web::to(|| {
            async {
                actori_rt::time::delay_for(Duration::from_millis(200)).await;
                Ok::<_, Error>(HttpResponse::Ok().body(STR))
            }
        })))
    });

    let connector = actoriwc::Connector::new()
        .connector(actori_connect::new_connector(
...
async fn test_connection_reuse() {
    ...
    let client = actoriwc::Client::default();
...
async fn test_connection_wait_queue() {
    ...
    let client = actoriwc::Client::build()
        .connector(actoriwc::Connector::new().limit(1).finish())
        .finish();
...
async fn test_no_decompress() {
    ...
    let mut res = actoriwc::Client::new()
        .get(srv.url("/"))
        .no_decompress()
        .send()
        .await
...
//     let mut sys = actori::System::new("test");
