...
use actori_http::http::header::{self, Header, IntoHeaderValue};
...
/// #[actori_rt::main]
/// async fn main() {
///    let response = actoriwc::Client::new()
...
impl ClientRequest {
    ...
    /// ```rust
    /// fn main() {
    /// # actori_rt::System::new("test").block_on(futures::future::lazy(|_| {
    ///     let req = actoriwc::Client::new()
    ///         .get("http://www.rust-lang.org")
    ///         .set(actoriwc::http::header::Date::now())
    ///         .set(actoriwc::http::header::ContentType(mime::TEXT_HTML));
    /// #   Ok::<_, ()>(())
    /// # }));
    /// }
    /// ```
    ...
    ///
    /// ```rust
    /// use actoriwc::{http, Client};
    ///
    /// fn main() {
    /// # actori_rt::System::new("test").block_on(async {
    ///     let req = Client::new()
    ///         .get("http://www.rust-lang.org")
    ///         .header("X-TEST", "value")
    ...
    /// #[actori_rt::main]
    /// async fn main() {
    ///     let resp = actoriwc::Client::new().get("https://www.rust-lang.org")
    ///         .cookie(
    ///             actoriwc::http::Cookie::build("name", "value")
...
mod tests {
    ...
    fn test_basics() {
        let mut req = Client::new()
            ...
            .if_true(true, |req| req.header(header::SERVER, "actoriwc"))
            .if_true(false, |req| req.header(header::EXPECT, "actoriwc"))
