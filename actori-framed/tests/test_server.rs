...
use actori_http::{body, http::StatusCode, ws, Error, HttpService, Response};
use actori_http_test::test_server;
use actori_service::{pipeline_factory, IntoServiceFactory, ServiceFactory};
use actori_utils::framed::Dispatcher;
use bytes::Bytes;
use futures::{future, SinkExt, StreamExt};

use actori_framed::{FramedApp, FramedRequest, FramedRoute, SendError, VerifyWebSockets};
...
async fn test_service() {
    let mut srv = test_server(|| {
        pipeline_factory(actori_http::h1::OneRequest::new().map_err(|_| ())).and_then(
            pipeline_factory(
