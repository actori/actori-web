//! Request identity service for Actori applications.
//!
...
use actori_web::dev::{Extensions, Payload, ServiceRequest, ServiceResponse};
use actori_web::error::{Error, Result};
use actori_web::http::header::{self, HeaderValue};
use actori_web::{FromRequest, HttpMessage, HttpRequest};

/// The extractor type to obtain your identity from a request.
///
/// ```rust
/// use actori_web::*;
/// use actori_identity::Identity;
///
...
/// ```rust
/// use actori_web::App;
/// use actori_identity::{CookieIdentityPolicy, IdentityService};
///
/// fn main() {
///     let app = App::new().wrap(IdentityService::new(
///         // <- create identity middleware
        ...
        async move {
            match fut.await {
                Ok(id) => {
                    ...
                    // https://github.com/actori/actori-web/issues/1263
...
mod tests {
    ...
    use super::*;
    use actori_service::into_service;
    use actori_web::http::StatusCode;
    use actori_web::test::{self, TestRequest};
    use actori_web::{error, web, App, Error, HttpResponse};

    const COOKIE_KEY_MASTER: [u8; 32] = [0; 32];
    const COOKIE_NAME: &'static str = "actori_auth";
    ...
    >(
        f: F,
    ) -> impl actori_service::Service<
        Request = actori_http::Request,
        Response = ServiceResponse<actori_web::body::Body>,
        Error = Error,
    > {
        test::init_service(
            App::new()
    ...
    async fn test_borrowed_mut_error() {
        ...
        let mut srv = IdentityServiceMiddleware {
            ...
            service: Rc::new(RefCell::new(into_service(|_: ServiceRequest| {
                async move {
                    actori_rt::time::delay_for(std::time::Duration::from_secs(100)).await;
        ...
        actori_rt::time::delay_for(std::time::Duration::from_millis(50)).await;
