...
use actori_web::dev::{RequestHead, ServiceRequest, ServiceResponse};
use actori_web::error::{Error, ResponseError, Result};
use actori_web::http::header::{self, HeaderName, HeaderValue};
use actori_web::http::{self, Error as HttpError, Method, StatusCode, Uri};
...
impl Cors {
    ...
    /// CredentialsWithWildcardOrigin` error during actori launch or runtime.
    ///
    /// Defaults to `false`.
...
#[cfg(test)]
mod tests {
    use actori_service::{fn_service, Transform};
